...
def create_star_schema():
    ...
    for result in results:
        search_time_query = """
            SELECT id FROM DENG_PRODUCTION.DATA_PROD.TIME_DIM WHERE year = %s AND month = %s AND day_type = %s AND hour = %s;
        """
        ...
        search_location_query = """
            SELECT id FROM DENG_PRODUCTION.DATA_PROD.LOCATION_DIM WHERE name = %s;
        """
        cursor_prod.execute(search_location_query, (result[4],))
        location_id = cursor_prod.fetchone()[0]
        search_incident_query = """
            SELECT id FROM DENG_PRODUCTION.DATA_PROD.INCIDENT_DIM WHERE name = %s;
        """
        cursor_prod.execute(search_incident_query, (result[5],))
        incident_id = cursor_prod.fetchone()[0]
        insert_query = """
            INSERT INTO DENG_PRODUCTION.DATA_PROD.FACT_TABLE (time_id, location_id, incident_id, avg_temperature, 
...
if __name__ == '__main__':
    ...
    if redis_client.exists(f'data_star_schema_{year}'):
        ...
        create_star_schema()
        # Cache data in Redis
        redis_client.set(f'data_star_schema_{year}', json.dumps({'status': 'done', 'duration': f'{time.time() - start_time}'}), timeout=60 * 60 * 24 * 7)
